...
export default async function PainelPage() {
  ...
  const { data: produtos } = loja
    ? await supabase
        .from("produtos")
        .select("id, nome, preco, categoria_id, tamanhos, imagem_url, destaque, mais_vendido, status")
        .eq("loja_id", loja.id)
        ...
        {loja && (
          <div className="mt-6 flex flex-wrap gap-6">
            <div>
              ...
              <p className="text-xs text-ink/50">produtos</p>
            </div>
            <div>
              <p className="text-2xl font-semibold">
                {(categorias ?? []).length}
              </p>
              <p className="text-xs text-ink/50">categorias</p>
            </div>
            <div>
              <p className="text-2xl font-semibold">
                {(produtos ?? []).filter((p) => p.status === "publicado").length}
              </p>
              <p className="text-xs text-ink/50">publicados</p>
            </div>
            <div>
              <p className="text-2xl font-semibold">
                {(produtos ?? []).filter((p) => p.status === "oculto").length}
              </p>
              <p className="text-xs text-ink/50">ocultos</p>
            </div>
          </div>
        )}
        ...
        {loja && (
          <>
            ...
            <div className="mt-14 pt-10 border-t border-line max-w-lg">
              ...
              <p className="mt-1 text-sm text-ink/60">
                Cadastre os produtos da sua loja, com foto, tamanhos e
                categoria.
              </p>
              <div className="mt-5">
                <ProdutosManager
